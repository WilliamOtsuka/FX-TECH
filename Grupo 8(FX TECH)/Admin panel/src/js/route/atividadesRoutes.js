...
let feedback = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, 'feedback/');
    },
    filename: (req, file, cb) => {
...
let uploadFeedback = multer({ storage: feedback });
...
router.post('/atividades/:id/correcao/aluno/:idAluno', uploadFeedback.single('arquivo'), AtividadesController.enviarCorrecao);
router.put('/atividades/:id/correcao/aluno/:idAluno', uploadFeedback.single('arquivo'), AtividadesController.atualizarCorrecao);
